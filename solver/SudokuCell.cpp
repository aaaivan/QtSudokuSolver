...
SudokuCell::SudokuCell(SudokuGrid* grid, unsigned short row, unsigned short col, unsigned short sudoku_size) :
	mParentGrid(grid),
	mValue(0),
	mRow(row),
	mCol(col),
	mId(col + sudoku_size * row),
	mViableOptions(),
	mEliminationHints(),
	mIsGiven( false )
{
	for (unsigned short i = 1; i <= sudoku_size; ++i)
	{
